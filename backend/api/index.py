...
# ── Settings ──────────────────────────────────────────────────────────────────
# All environment-specific values loaded from config.py (pydantic-settings).
# In production, override any setting via Render's Environment Variables panel
# without touching code. Falls back to sensible defaults for local development.
...
# Log level is environment-configurable: INFO in production, DEBUG locally.
...
# Limit is environment-configurable (default: 30/minute per IP).
# In production this would be tighter and tied to authenticated API keys.
limiter = Limiter(key_func=get_remote_address)

# ── Artifact Loading ──────────────────────────────────────────────────────────
# Paths come from settings (pydantic-settings) so they can be overridden
# via environment variables without code changes — e.g. pointing to S3
# in a production deployment instead of committed pickle files.
#
...
# model_meta    — version, AUC, hyperparameters — single source of truth
#                 for everything surfaced at the /metadata endpoint
model         = joblib.load(settings.model_path)
train_medians = json.load(open(settings.medians_path))
model_meta    = json.load(open(settings.meta_path))
logger.info(
...
# Attach rate limiter and its 429 Too Many Requests exception handler
app.state.limiter = limiter
app.add_exception_handler(RateLimitExceeded, _rate_limit_exceeded_handler)

# CORS: origins loaded from settings (default: ["*"] — open for demo).
# For an iOS-only client, CORS headers are technically unnecessary
# (iOS is not a browser), but keeping it open simplifies curl and
# Swagger UI testing. In production, restrict to trusted origins via
...
# Surfaced in /metadata so API consumers can see what drives the score.
FEATURES = [
...
def metadata():
    ...
    Data is loaded from model_metadata.json at startup — the single source
    of truth for all model information. This pattern means the /metadata
    response automatically reflects any future retraining without code changes.
    ...
    this endpoint sees immediately that this is a demo, not a production scorer.
    """
    return {
        **model_meta,
        "disclaimer": (
...
def predict(request: Request, data: CreditInput):
    ...
    Rate limit is environment-configurable via settings.rate_limit.
    ...
    try:
        ...
        # Column order must match FEATURES list above and the training pipeline.
        features = np.array([[
            data.age_years,
            data.years_employed,
